#!/bin/bash
# 发布巡检 Agent 唤醒脚本
# 监控 tasks/ 目录，发现巡检请求时唤醒 deploy-monitor

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/harness-presets.sh"

TASKS_DIR="$ROOT_DIR/tasks"
AGENT_SESSION="deploy-monitor"
...
main() {
    ...
    local task_count
    task_count=$(find "$TASKS_DIR" -type f -name 'deploy-req-*.json' 2>/dev/null | wc -l | tr -d ' ')

    if [ "$task_count" -eq 0 ]; then
        return
    fi

    local cooldown_file="/tmp/deploy_watcher_cooldown"
    ...
    tmux send-keys -t "$AGENT_SESSION" "读tasks/deploy-req并巡检发布"
